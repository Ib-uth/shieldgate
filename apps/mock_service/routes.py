...
from .main import verify_token, require_role, require_minimum_role
...
MOCK_DATA = {
    "posts": [
        {"id": "1", "title": "First Post", "content": "This is the first post", "author": "user-456"},
        {"id": "2", "title": "Second Post", "content": "This is the second post", "author": "user-456"},
        {"id": "3", "title": "Admin Post", "content": "This is an admin post", "author": "admin-123"},
    ],
    "users": [
        {"id": "user-456", "name": "Regular User", "email": "user@shieldgate.dev", "role": "user"},
        {"id": "readonly-789", "name": "Readonly User", "email": "readonly@shieldgate.dev", "role": "readonly"},
        {"id": "admin-123", "name": "Admin User", "email": "admin@shieldgate.dev", "role": "admin"},
    ],
    "settings": {
...
async def get_user_profile(user: Dict[str, Any] = Depends(require_minimum_role("user"))):
...
async def test_error(status_code: int):
    ...
    raise HTTPException(
        ...
        detail=f"Test error with status code {status_code}"
    )
