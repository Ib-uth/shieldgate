...
from .middleware.auth import JWTMiddleware, JWTAuthenticator
...
def setup_routes():
    ...
    app.add_route("/proxy/{path:path}", proxy_handler, methods=["GET", "POST", "PUT", "DELETE", "PATCH"])
    
    # Catch-all proxy for any other path
    app.add_route("/{path:path}", catch_all_handler, methods=["GET", "POST", "PUT", "DELETE", "PATCH"])
