...
from ..models.database import RequestLog, BlockedIP, Metrics, SessionLocal
from ..models.schemas import HealthResponse, MetricsResponse
from ..routes.proxy import RequestProxy
    ...
    async def get_metrics(
        ...
        try:
            ...
            try:
                ...
                ).group_by(RequestLog.ip).order_by(
                    func.count(RequestLog.id).desc()
                ).limit(10).all()
                
                top_blocked_ips = [
                    {"ip": ip, "blocked_count": count}
                ...
                threat_score_distribution = {
                    level: count for level, count in threat_scores
                }
    ...
    async def get_blocked_ips(limit: int = 100):
        ...
        try:
            ...
            try:
                ...
                ).order_by(BlockedIP.blocked_at.desc()).limit(limit).all()
