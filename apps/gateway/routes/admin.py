...
from ..models.database import RequestLog, BlockedIP, SessionLocal
        ...
        try:
            ...
            try:
                ...
                for period_name, start_time in [
                    ...
                    blocked_requests = db.query(RequestLog).filter(
                        RequestLog.timestamp >= start_time,
                        RequestLog.blocked == True
                    ).count()
                    
                    unique_ips = db.query(RequestLog.ip).filter(
                        RequestLog.timestamp >= start_time
                    ).distinct().count()
                    ...
                    avg_threat_score = db.query(func.avg(RequestLog.threat_score)).filter(
                        RequestLog.timestamp >= start_time
                    ).scalar() or 0
                ...
                top_threat_actors = db.query(
                    RequestLog.ip,
                    func.avg(RequestLog.threat_score).label('avg_threat_score'),
                    func.count(RequestLog.id).label('request_count'),
                    func.sum(func.cast(RequestLog.blocked, int)).label('blocked_count')
                ).filter(
                    RequestLog.timestamp >= last_day
                ).group_by(RequestLog.ip).having(
                    func.avg(RequestLog.threat_score) > 0.5
                ).order_by(
                    desc('avg_threat_score')
                ).limit(10).all()
                
                # Recent blocked IPs
                recent_blocked = db.query(BlockedIP).filter(
                    BlockedIP.blocked_at >= last_day,
                    BlockedIP.unblocked_at.is_(None)
                ).order_by(BlockedIP.blocked_at.desc()).limit(20).all()
    ...
    async def unblock_ip_address(ip: str):
        ...
        try:
            ...
            try:
                ...
                blocked_ip.unblocked_by = "admin"  # In a real system, this would be the current user
                db.commit()
    ...
    async def get_blocked_ips(limit: int = 100, active_only: bool = True):
        ...
        try:
            ...
            try:
                ...
                blocked_ips = query.order_by(BlockedIP.blocked_at.desc()).limit(limit).all()
    ...
    async def get_threat_score_distribution(hours: int = 24):
        ...
        try:
            ...
            try:
                ...
                high_threat_requests = db.query(RequestLog).filter(
                    RequestLog.timestamp >= start_time,
                    RequestLog.threat_score >= 0.7
                ).order_by(desc(RequestLog.threat_score)).limit(50).all()
                    ...
                    "high_threat_requests": [
                        {
                            ...
                            "blocked": req.blocked,
                            "user_agent": req.user_agent[:100] if req.user_agent else None
                        }
