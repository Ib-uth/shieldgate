...
class ThreatDetectionModel:
    ...
    def prepare_features(self, features: ThreatFeatures) -> np.ndarray:
        ...
        endpoint_numeric = self.endpoint_categories.get(
            features.endpoint_category, self.endpoint_categories["unknown"]
        )
...
def generate_synthetic_training_data(n_samples: int = 10000) -> list[dict[str, Any]]:
    ...
    for i in range(n_samples):
        ...
        endpoint_category = np.random.choice(
            endpoint_categories, p=[0.4, 0.3, 0.1, 0.1, 0.1]
        )
...
def train_threat_detection_model(
    training_data: list[dict[str, Any]], model_path: str
    ...
    print(
        f"Model training complete. Test score range: {test_scores.min():.3f} to {test_scores.max():.3f}"
    )
