...
- **`DOWNSTREAM_URL` and system health**: Point **`DOWNSTREAM_URL`** at your **real** backend’s origin (no path). The gateway probes **`GET {DOWNSTREAM_URL}/health`**; that route must return **200** on your live API. If it 404s or is unreachable, the admin **System Health** panel shows downstream failure and overall status may be **degraded**, even when Redis and the database are fine. Add or expose **`GET /health`** on the service you already run behind the gateway, then redeploy the gateway with the updated URL.

- **Vercel downstream template**: This repo includes a minimal Next.js app at [`apps/downstream-vercel`](apps/downstream-vercel) with **`GET /health`** and **`GET /api/v1/ping`**. Deploy it to Vercel (set the project **Root Directory** to `apps/downstream-vercel`), then set **`DOWNSTREAM_URL`** to the Vercel URL. See [`apps/downstream-vercel/README.md`](apps/downstream-vercel/README.md) for steps and curl checks.
